...
def store(pump_array, detector_array, feedback_array, output_dirpath):
    output_dict_for_json = {
        'pump_array': {ak.to_list(pump_array[field]) for field in pump_array.fields},
        'detector_array': {ak.to_list(detector_array[field]) for field in detector_array.fields},
        'feedback_array': {ak.to_list(feedback_array[field]) for field in feedback_array.fields},
    }

    if not os.path.exists(output_dirpath):
        os.makedirs(output_dirpath)
    with open(os.path.join(output_dirpath, output_dict_for_json), 'w') as f:
        json.dump(f)
...
def main(n_steps, output_dirpath):
    ...
    for step in range(n_steps):
        print(f"{'='*60}\nstep: {step}\n{'='*60}")
        if step == 0:
            feedback_array.append(np.array([vacuum_noise()]))
            detector_array.append(np.array([vacuum_noise()]))
